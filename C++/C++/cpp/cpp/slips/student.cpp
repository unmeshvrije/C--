...
student::~student ()
{
// roll,name of student object 
// which is getting destroyed
  fp <<"\n"<< roll << "\t" << name << "\t" << p << "\t" << fe << "\t" << be;
  int temp = fp.rdstate();
  cout << endl <<"after destructor = " <<temp;
}
...
int main ()
{
  ...
  student *s;
  fp.open ("student.txt", ios::in | ios::out);  //existing file
  temp = fp.rdstate();
  cout <<endl <<" Initially rdstate()= " << temp <<endl;
  fp >> n; // no of records in file
  ...
  fp.close ();
temp=fp.rdstate();
cout<<"\nafter rdstate () t="<<temp;
getchar();
  fp.open("student.txt",ios::out);
temp=fp.rdstate();
cout<<"\nafter opening in out mode t="<<temp;
getchar();
 fp << n;
cout<<"calling destructor..............";
//we have 'n' student objects delete s
